...
<div  class="col-sm-9 col-sm-offset-3 col-lg-7 col-lg-offset-4 main" >
    <div class="row">
        <div class="col-lg-12" align="center">
            ...
            <p style="font-size:18px;">Preferred City One (I)</p>
        </div>
    </div>
    ...
    <form method="post">
        ...
        $postsq = "SELECT * FROM `posts` ";
            ...
            <?php
                    $postscount2q = "SELECT COUNT(d_centerid) FROM `details` JOIN post_apply ON city_prefer= '$id2' AND d_said = said WHERE  d_centerid IS NOT NULL AND d_postid = '$posts1' AND d_status ='Approved'";
            ...
            <div class="row" style="">
                ...
                <div class="col-lg-2">
                    <?php
                    $postscountq = "SELECT COUNT(d_postid) FROM `details` JOIN post_apply ON city_prefer= '$id2' AND d_said = said WHERE d_postid = '$posts1' AND d_status ='Approved'";
 ...
 for ($i = 0; $i <= $posttotalc; $i++) {
    $postdyn = $post1[$i];
    echo $postdyn;
    $query1 = "UPDATE details
               JOIN post_apply ON details.d_said = post_apply.said
               SET details.d_centerid = '$id'
               WHERE details.d_said IN (
                   SELECT d_said
                   FROM (
                       SELECT details.d_said
                       FROM details
                       JOIN post_apply ON details.d_said = post_apply.said
                       WHERE city_prefer = '$id3'
                         AND details.d_postid = '{$post2[$i]}'
                         AND details.d_status = 'Approved' 
                         AND details.d_centerid IS NULL
                       LIMIT $postdyn
                   ) AS derived
               )
               AND details.d_postid = '{$post2[$i]}';"; // Ensure the specific post is updated
...
//$mail->isSMTP();
//$mail->CharSet = 'UTF-8';
//$mail->Host = 'mail.fgeirecruitment.com';
//$mail->SMTPAuth = true;
//$mail->SMTPSecure = 'ssl';
//$mail->Port = 465;
//$mail->isHTML(true);
...
//$mail->addAddress('contact@fgeirecruitment.com');
        ...
        //    $mail->Subject = 'FGEI(C/G) Recruitment - Roll Number Slip';
          //  $mail->Body    = 'Dear Candidate, Your roll number slip is issued please login to FGEI (C/G) Recruitment portal and download your roll number slip. Thankyou';
       //     if(!$mail->send())
       //     {
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/fancybox/3.5.7/jquery.fancybox.min.js" integrity="sha512-uURl+ZXMBrF4AwGaWmEetzrd+J5/8NRkWAvJx5sbPSSuOb0bZLqf+tOzniObO00BjHa/dD7gub9oCGMLPQHtQA==" crossorigin="anonymous">
