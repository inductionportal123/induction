        ...
        <div class="bg-white p-8 rounded-xl shadow-lg">
            ...
            <form id="post_apply_form" class="space-y-8">
                ...
                <div class="card p-6 rounded-lg">
                    ...
                    <?php
// Check if user has qualifications (use prepared statement to prevent SQL injection)
$quali = "SELECT bs_title, bs16_title, ms_title, primary_title 
          FROM qualification 
          WHERE said = ?";
$stmt = mysqli_prepare($conn, $quali);
mysqli_stmt_bind_param($stmt, "s", $userid);
mysqli_stmt_execute($stmt);
$exequali = mysqli_stmt_get_result($stmt);
    ...
    if ($qualirow > 0) {
        if (empty($eligible_degrees)) {
            ...
            $newquery = $user_gender == 'transgender'
                ? "SELECT p.pid, p.name, p.gender, p.bps, p.professional_degree_required 
                   FROM posts p
            ...
            if (mysqli_num_rows($newexe) > 0) {
                ...
                while ($rows = mysqli_fetch_array($newexe)) {
                    ...
                    echo "<label class='checkbox-item'><input type='checkbox' name='post_apply[]' value='{$rows['pid']}' data-pro-degree='{$rows['professional_degree_required']}' $checked> " . strtoupper(htmlspecialchars($rows['name'])) . " (<small>" . strtoupper(htmlspecialchars($rows['gender'])) . ", BPS-{$rows['bps']}</small>)</label>";
                }
                echo '</div>';
            } else {
