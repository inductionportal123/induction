...
$checkHead=0;
    $sql="SELECT * FROM `per_info`,`emp_document`,`post_apply`,`details` WHERE emp_document.said=per_info.said AND post_apply.said=emp_document.said AND details.d_status ='Rejected'AND post_apply.said=details.d_said";
...
if ($row['revert'] == 1) {
    $output .= '<td>Reverted</td>';
} else {
    $output .= '<td>Rejected</td>';
}
