        ...
        <div class="bg-white p-8 rounded-xl shadow-lg">
            ...
            <form id="post_apply_form" class="space-y-8">
                ...
                <div class="card p-6 rounded-lg">
                    ...
                    <div class="mb-6">
                        ...
                        if ($qualirow > 0) {
                            $newquery = $user_gender == 'transgender' 
                                ? "SELECT `pid`,`name`,`gender` FROM `posts` WHERE posts.cat = 3 AND posts.pid IN (SELECT post_details.pid FROM post_details WHERE post_details.req_deg IN ('Primary', 'Middle', 'Matric', 'Inter', 'Bachelors', 'Bachelors16'))"
                                : "SELECT `pid`,`name`,`gender` FROM `posts` WHERE posts.cat = 3 AND posts.pid IN (SELECT post_details.pid FROM post_details WHERE post_details.req_deg IN ('Primary', 'Middle', 'Matric', 'Inter', 'Bachelors', 'Bachelors16')) AND (posts.gender = '$user_gender' OR posts.gender = 'both')";
                    ...
                    <!-- Category II: Non-Teaching -->
                    <div class="mb-6">
                        <h4 class="text-teal-600 font-semibold mb-2">Category II: Non-Teaching (BPS 6-15) | زمرہ II: غیر تدریسی عملہ</h4>
                        <?php
                        if ($qualirow > 0) {
                            $newquery = $user_gender == 'transgender'
                                ? "SELECT `pid`,`name`,`gender` FROM `posts` WHERE posts.cat = 1 AND posts.pid IN (SELECT post_details.pid FROM post_details WHERE post_details.req_deg IN ('Primary', 'Middle', 'Matric', 'Inter', 'Bachelors', 'Bachelors16'))"
                                : "SELECT `pid`,`name`,`gender` FROM `posts` WHERE posts.cat = 1 AND posts.pid IN (SELECT post_details.pid FROM post_details WHERE post_details.req_deg IN ('Primary', 'Middle', 'Matric', 'Inter', 'Bachelors', 'Bachelors16')) AND (posts.gender = '$user_gender' OR posts.gender = 'both')";
                            $newexe = mysqli_query($conn, $newquery);
                            if (mysqli_num_rows($newexe) > 0) {
                                echo '<div class="checkbox-group">';
                                while ($rows = mysqli_fetch_array($newexe)) {
                                    $checked = !empty($post_apply_data) && in_array($rows["pid"], $post_apply_data) ? 'checked' : '';
                                    echo "<label class='checkbox-item flex items-center'><input type='checkbox' name='post_apply2[]' value='{$rows['pid']}' class='mr-2' $checked> " . strtoupper($rows['name']) . " (<small>" . strtoupper($rows['gender']) . "</small>)</label>";
                                }
                                echo '</div>';
                            }
                        } else {
                    ...
                    <?php
if ($qualirow > 0) {
    // Updated query to fetch district from per_info_contact and spouse info from per_info_spouse
    $query_two = "SELECT pic.contact_district, pis.female_applying, pis.female_husband_district 
                  FROM per_info pi
    ...
    if ($query_row_two = mysqli_fetch_array($query_exe_two)) {
        // Decide the correct district based on 'female_applying'
        $user_district = ($query_row_two['female_applying'] == '1')
                         ? $query_row_two['female_husband_district']
                         : $query_row_two['contact_district'];
    } else {
        // If no row is fetched, you can either set a default or display an error message
        $user_district = '';
        echo '<p class="text-red-600">User district data not found. Please update your contact information.</p>';
    }

    // Continue only if a valid district exists
    if (!empty($user_district)) {
        // Normalize district values
        $valid_districts = ['mardan', 'kohat', 'hangu', 'peshawar', 'nowshera', 'attock', 'islamabad', 'rawalpindi', 'haripur', 'abbottabad', 'jhelum', 'kotli', 'muzaffarabad', 'mianwali', 'khushab', 'mandi_bahauddin', 'gujrat', 'gujranwala', 'sialkot', 'lahore', 'jhang', 'multan', 'hyderabad', 'karachi', 'quetta'];
        
        if (strpos($user_district, 'karachi') !== false) {
        ...
        if (in_array($user_district, $valid_districts)) {
            $sql = $user_gender == 'transgender'
                ? "SELECT p.pid, p.name, p.gender, ls.lsid, ls.$user_district 
                   FROM posts p 
                   JOIN lowerstaff ls ON p.pid = ls.pid 
                   WHERE p.cat = 2 
                     AND p.pid IN (SELECT post_details.pid FROM post_details WHERE post_details.req_deg IN ('Primary', 'Middle', 'Matric')) 
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    $checked = (!empty($post_apply_data) && in_array($row["pid"], $post_apply_data)) ? 'checked' : '';
                    echo "<label class='checkbox-item flex items-center'><input type='checkbox' name='post_apply3[]' value='{$row['pid']}' class='mr-2' $checked> " . strtoupper($row['name']) . " (<small>" . strtoupper($row['gender']) . "</small>)</label>";
                }
                echo '</div>';
            } else {
                echo '<p class="text-red-600">No posts available for your district.</p>';
            }
        } else {
            echo '<p class="text-red-600">Your district is not available in the list.</p>';
        }
    }
} else {
    echo '<p class="text-red-600">Please add <b>Qualifications</b> first.</p>';
